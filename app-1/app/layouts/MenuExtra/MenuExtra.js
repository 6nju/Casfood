...
class MenuExtra extends Component {
    constructor(props) {
        super(props)
        this.state = {
		  ...
		  <View style={styles.container}>
							...
							<View style={ [styles.formGroup, {backgroundColor: '#fff', height: 40}] }>
								 <Picker 
								  ...
								  onValueChange={(itemValue, itemIndex) => { 
										let data = this.state.menus[itemValue]
									...
									<Picker.Item value='-1' label='Chọn người' />
									{
										this.state.menus.map((val_, index_) => {
											return (
												<Picker.Item label={val_.name} value={index_} />
											)
										})
									}
								</Picker>
			...
			<View style={[{width: width,  marginBottom: 10, marginTop: -4, height: 50, backgroundColor: '#ddd', position: 'relative'}]} >
				...
				<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 10, width: width - 20}]}>
				    <Text maxFontSizeMultiplier={1} style={{fontWeight: 'bold'}}>Tổng năng lượng bạn cần là:</Text> {parseFloat((this.state.tee*0.8)).toFixed(0)} đến {parseFloat((this.state.tee*1.2)).toFixed(0)} Kcal
				</Text>
			</View>	
			...
			<View style={{backgroundColor: '#ddd', paddingTop: 10, paddingBottom: 5}}>
				{
					this.state.items[this.state.key].items.map((val, index) => {
						...
						if(val.math != 2 && val.items.length > 0){
							return (
								<View>
							...
							return (
								...
								this.state.data['m2'].map((val_, index_) => {
									...
									<View style={[styles.itemList, {backgroundColor: '#fff'}]}>
										...
										<Text maxFontSizeMultiplier={1} style={{fontWeight: 'bold'}}>+</Text>
										</View>
									</View>
									{ 
										...
										<View style={[{width: width,  height: 50, backgroundColor: '#E7F0E4', position: 'relative'}]} >
												<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 20}]}>
													{val_.value.gradient1 + ': ' + parseInt(val_.value.amount1*this.state.data['m2'][index_].he_so) + 'g'}
												</Text>
										</View>
										:
										null
									...
									{ 
										...
										<View style={[{width: width,  height: 50, backgroundColor: '#E7F0E4', position: 'relative'}]} >
												<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 20}]}>
													{val_.value.gradient2 + ': ' + parseInt(val_.value.amount2*this.state.data['m2'][index_].he_so) + 'g'}
												</Text>
										</View>
										:
										null
									...
									{ 
										...
										<View style={[{width: width,  height: 50, backgroundColor: '#E7F0E4', position: 'relative'}]} >
												<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 20}]}>
													{val_.value.gradient3 + ': ' + parseInt(val_.value.amount3*this.state.data['m2'][index_].he_so) + 'g'}
												</Text>
										</View>
										:
										null
									...
									{ 
										...
										<View style={[{width: width,  height: 50, backgroundColor: '#E7F0E4', position: 'relative'}]} >
												<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 20}]}>
													{val_.value.gradient4 + ': ' + parseInt(val_.value.amount4*this.state.data['m2'][index_].he_so) + 'g'}
												</Text>
										</View>
										:
										null
									...
									{ 
										...
										<View style={[{width: width,  height: 50, backgroundColor: '#E7F0E4', position: 'relative'}]} >
												<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 20}]}>
													{val_.value.gradient5 + ': ' + parseInt(val_.value.amount5*this.state.data['m2'][index_].he_so) + 'g'}
												</Text>
										</View>
										:
										null
...
export default connect(mapStateToProps)(MenuExtra)
...
class MenuGroup extends Component {
    constructor(props) {
        ...
        apis.getMenuGroup(this.state.phone, 1)
                .then(res => {
					...
					this.setState({ process: false, groups: res.data.group,item_group:res.data.group[0].items,length: res.data.group[1].items.length,type: res.data.type, isShowP: true, items: res.data.items,items_: res.data.items, data_:res.data.items[0].data, data:res.data.items[0].data, stt:res.data.items[0].stt, tee: ((res.data.tee * res.data.hesotee)/100)})
	...
	_showTabGroup(id) {
		...
		if(id){
			if(this.state.length == 0){
						Alert.alert(
					'Thông báo',
					'Để lên được thực đơn nhóm bạn cần tạo nhóm',
				  [
					{
					  text: 'Tạo nhóm',
					  onPress: () => {
	...
	pickClick(id) {
		...
		for(let i = 0; i < data[index_].length; i++){
			...
			if(this.state.items[this.state.key].items[this.state.keyData].math == 10){
				data[index_][i].he_so = 1.5/data[index_].length;
		...
		for(let i = 0; i < data[index_].length; i++){
			...
			if(b == this.state.key_one){
				...
				if(this.state.type == 0){
					...
					if(this.state.items[this.state.key].items[this.state.keyData].math == 12){
				protein = this.state.groups[this.state.key_].tee[b]*hesoman*0.18*0.8*2/3
			}
			if(this.state.items[this.state.key].items[this.state.keyData].math == 11){
				protein = this.state.groups[this.state.key_].tee[b]*hesoman*0.18*0.8
			...
			if(this.state.groups[this.state.key_].users[b].age <  16){
				if(this.state.groups[this.state.key_].users[b].age >=  3 && this.state.groups[this.state.key_].users[b].age <=  8){
					heso = ((protein*(this.state.groups[this.state.key_].users[b].hesoman/100))/lenth)/data[index_][i].value.Protein;
				}else{
					heso = ((protein*(this.state.groups[this.state.key_].users[b].hesoman/100))/lenth)/data[index_][i].value.Protein;
				}
  ...
  barPercentage: 0.4
		...
		return (
		  <View style={styles.container}>
			...
			<ScrollView>
			<TouchableOpacity style={{position: 'relative', marginTop: 0,width: width, height: 300*(width)/1000,marginLeft:0, backgroundColor:'#FFA800'}}>

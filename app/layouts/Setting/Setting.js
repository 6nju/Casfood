...
class Setting extends Component {
    constructor(props) {
        ...
        this.state = {
			...
			tee_: (this.props.user_login.hesotee) ? this.props.user_login.hesotee : 100,
			tee: 0,
			t: 0,
			l: 0,
			items: [],
	...
	_register = () => {
			...
			const {name, age, height_,job, tee_,sick, playId, cannang,weight, sex, phone, current_job, timkiem, data, mangthai, current_sport } = this.state
			sports = JSON.stringify(data)
		
		this.setState({ process: true }, () => {
            apis.editDinhduong(phone, sick, tee_)
                .then(res => {
					this.setState({ process: false })
	...
	changeExpandedD = () => {
	  ...
	  _plus = () => {
		let tee = parseInt(this.state.tee_);
		tee = tee + 10
		this.setState({
			tee_: tee
		})
		...
		return (
		  ...
		  <StatusBar backgroundColor="#90D077"/>
			...
			<TouchableOpacity  style={[styles.image_box]}>
				...
				<Image
					style={styles.image}
                    source={ require('./imgs/dinhduong.jpg') }
                />
				...
				return (
					...
					<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 40}]}>
						...
						<View style={[styles.itemList, {backgroundColor: '#fff'}]}>
								...
								<View style={{ marginTop: 10, height: 30, position: 'relative', width: width, borderColor: '#757F8C',borderBottomWidth: 1 }}>
								<Text maxFontSizeMultiplier={1} style={[styles.itemListText_,{marginLeft: 10}]}>
									<Text maxFontSizeMultiplier={1} style={{fontWeight: 'bold', paddingLeft: 10}}>Điều chỉnh hệ số năng lượng</Text>
								</Text>
								<TouchableOpacity style={{position:'absolute', top: -5,right: 10, zIndex: 1000}}   onPress={this._plus}>
								...
								<View style={{position:'absolute', top: -2,right: 13}}>
									<Text maxFontSizeMultiplier={1} style={{ width: 100, height: 30,borderRadius: 15,textAlign:'center', color: '#000'}}>{this.state.tee_}%</Text>
								</View>
								<TouchableOpacity style={{position:'absolute', top: -5,right: 83}} onPress={this._minus}>
									<Text maxFontSizeMultiplier={1} style={{fontWeight: 'bold', fontSize: 20,width: 30, height: 30, backgroundColor:'#90D077', borderRadius: 15,textAlign:'center', color: '#fff'}}>-</Text>
								</TouchableOpacity>

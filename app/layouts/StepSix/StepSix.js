...
class StepSix extends Component {
	...
	_next = () => {
		const { job, sick, sport, play, hour, phone, radioSelected, _w, data, sick_ } = this.state
		let level = 0
		...
		this.setState({ process: true }, () => {
            apis.stepsix(job, sick, sports, level, hour, phone, radioSelected, _w, sick_)
                .then(res => {
					this.setState({ process: false })
		...
		return (
		  ...
		  <ScrollView style={[{width: width, height: height - 85}]}>	
			...
			<View style={[{width: width,  marginBottom: 5, marginTop: -4, height: 60, backgroundColor: '#ddd', position: 'relative'}]} >
				...
				<Text maxFontSizeMultiplier={1} style={[styles.title, {color: '#757F8C', fontSize: 15, bottom: 7, left: 40, width: width - 40, fontWeight:'bold'}]}>
				    Thiết lập chế độ dinh dưỡng của bạn?
				</Text>
			</View>	
			...
			sicks_.map((val) => {
				return (
					...
					<TouchableOpacity  style={ [styles.itemInput, {position: 'relative', marginTop: 10,height: 30, width: width}] } key={val.id} onPress={this.sickClick_.bind(this, val.id)}>
					<View style={{
					  height: 20,
					  width: 20,
					  borderRadius: 10,
